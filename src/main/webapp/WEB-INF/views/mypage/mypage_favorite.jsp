		    ...
		    <div class="text-center">
		       <c:if test="${not empty favTourList}">
		       	<c:if test ="${startPage >pageBlock }">
		       		[<a href="mypage_favorite_list.do?pageNum=${startPage-1}">이전</a>]
		       	</c:if>
		       	<c:forEach var ="i" begin="${startPage}" end="${endPage}">
		       		[<a href ="mypage_favorite_list.do?pageNum=${i}">${i}</a>]
		       	</c:forEach>     	
		        <c:if test="${pageCount> endPage}">
		        	[<a href="mypage_favorite_list.do?pageNum=${endPage+1}">다음</a>]    
		       	</c:if>
		       </c:if>
		       <c:if test="${not empty favRouteList}">
		       	<c:if test ="${startPage >pageBlock }">
